...
def root(settings, path, create=True, mode=0644):
    """
    Append an absolute path to settings.__path__
    """
    parts = _name_to_parts(settings.__name__)
...
def home(settings, path=None, create=True, mode=0644, xdg=True):
    """
    Append a path to settings.__path__ derived from $XDG_CONFIG_HOME
    """
...
def default(settings, path=None, create=True, mode=0644):
    """
    Append a path to settings.__path__ derived from settings.__file__
    """
    path_file = os.path.abspath(settings.__file__)
